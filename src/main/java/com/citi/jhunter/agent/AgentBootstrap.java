package com.citi.jhunter.agent;
...
public class AgentBootstrap {
    ...
    private static final String ARTHAS_CORE_JAR = "JHunter-core.jar";
    private static final String ARTHAS_BOOTSTRAP = "com.citi.jhunter.core.server.ArthasBootstrap";
    ...
    // 全局持有classloader用于隔离 JHunter 实现
     ...
     * 让下次再次启动时有机会重新加载
     */
    public synchronized static void resetJHunterClassLoader() {
    ...
    private static ClassLoader loadOrDefineClassLoader(File arthasCoreJarFile) throws Throwable {
        if (arthasClassLoader == null) {
            arthasClassLoader = new JHunterClassloader(new URL[]{arthasCoreJarFile.toURI().toURL()});
        }
    ...
    private static synchronized void main(String args, final Instrumentation inst) {
        try {
            ps.println("JHunter server agent start...");
            if (args == null) {
                args = "";
            }
            args = decodeArg(args);

            String jhunterCoreJar;
            final String agentArgs;
            int index = args.indexOf(';');
            if (index != -1) {
                jhunterCoreJar = args.substring(0, index);
            ...
            File arthasCoreJarFile = new File(jhunterCoreJar);
            if (!arthasCoreJarFile.exists()) {
                ps.println("Can not find jhunter-core jar file from args: " + arthasCoreJarFile);
